...
const getMovieTrailer = async (req, res, next) => {
  try {
    ...
    // If movie already has a trailer URL, return it
    if (movie.trailer) {
    ...
    // Try to find trailer using YouTube service
    if (youtubeService.isEnabled()) {
      try {
        const trailer = await youtubeService.findBestTrailer(movie.title, movie.year);

        if (trailer) {
          // Optionally save the trailer URL to the movie for future use
          movie.trailer = trailer.url;
          movie.trailerEmbed = trailer.embedUrl;
          await movie.save();
        ...
        console.error('YouTube trailer search failed:', youtubeError);
      }
    }

    // Try TMDB if YouTube fails
    if (tmdbService.isEnabled() && movie.tmdbId) {
      try {
        const trailerUrl = await tmdbService.getMovieTrailer(movie.tmdbId);

        if (trailerUrl) {
          // Save the trailer URL to the movie
        ...
        console.error('TMDB trailer search failed:', tmdbError);
      }
    }
...
// @desc    Enrich movie with external data
// @route   PUT /api/movies/:id/enrich
// @access  Private (Admin only)
const enrichMovieData = async (req, res, next) => {
  try {
    const movie = await Movie.findById(req.params.id);
    ...
    if (tmdbService.isEnabled()) {
      try {
        ...
        if (movie.tmdbId) {
          ...
          tmdbMovie = await tmdbService.getMovieDetails(movie.tmdbId);
        } else {
          // Search for movie on TMDB
          const searchResult = await tmdbService.findMovieByTitleAndYear(movie.title, movie.year);
          if (searchResult) {
            tmdbMovie = await tmdbService.getMovieDetails(searchResult.id);
            enrichedData.tmdbId = searchResult.id.toString();
          }
        }

        if (tmdbMovie) {
          const convertedData = tmdbService.convertToFlickNetFormat(tmdbMovie);

          // Only update fields that are empty or need updating
          if (!movie.poster && convertedData.poster) enrichedData.poster = convertedData.poster;
          if (!movie.backdrop && convertedData.backdrop) enrichedData.backdrop = convertedData.backdrop;
          if (!movie.description && convertedData.description) enrichedData.description = convertedData.description;
          if (!movie.runtime && convertedData.runtime) enrichedData.runtime = convertedData.runtime;

          // Get cast and crew
          const credits = await tmdbService.getMovieCredits(tmdbMovie.id);
          if (credits.cast && credits.cast.length > 0) {
            enrichedData.cast = credits.cast.slice(0, 10).map(actor => actor.name);
          }
          if (credits.crew) {
            const director = credits.crew.find(person => person.job === 'Director');
            if (director) enrichedData.director = director.name;
          }
        }
      } catch (tmdbError) {
    ...
    // Enrich with YouTube trailer if not already present
    if (youtubeService.isEnabled() && !movie.trailer) {
      try {
        const trailer = await youtubeService.findBestTrailer(movie.title, movie.year);
        if (trailer) {
          enrichedData.trailer = trailer.url;
          enrichedData.trailerEmbed = trailer.embedUrl;
        }
      } catch (youtubeError) {
        console.error('YouTube enrichment failed:', youtubeError);
      }
    }
    ...
    if (Object.keys(enrichedData).length > 0) {
      ...
      res.status(200).json({
        success: true,
        data: movie,
        message: 'No additional data found to enrich movie'
      });
    }
  } catch (error) {
    next(error);
