...
const Dashboard = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      // Try to load trending movies from TMDB API first
      let trendingData;
      try {
        console.log('📡 Calling TMDB trending movies API...');
        const tmdbTrendingResult = await externalApiService.getTMDBTrendingMovies('week', 1);
        console.log('📊 TMDB trending result:', tmdbTrendingResult);
        if (tmdbTrendingResult.success && tmdbTrendingResult.movies.length > 0) {
          trendingData = { movies: tmdbTrendingResult.movies };
          console.log('✅ Using TMDB trending movies data:', trendingData.movies.length, 'movies');
        } else {
          throw new Error('TMDB API returned no data');
        }
      } catch (tmdbError) {
        console.warn('⚠️ TMDB API failed, falling back to sample data:', tmdbError.message);
        setUsingFallbackData(true);
        trendingData = await movieService.getTrendingMovies();
        console.log('📦 Using fallback data:', trendingData);
      }

      setTrendingMovies(trendingData.movies);

      // Try to load upcoming movies from TMDB API
      let upcomingData;
      try {
        const tmdbUpcomingResult = await externalApiService.getTMDBUpcomingMovies(1);
        if (tmdbUpcomingResult.success && tmdbUpcomingResult.movies.length > 0) {
          upcomingData = tmdbUpcomingResult.movies.slice(0, 4);
          console.log('✅ Using TMDB upcoming movies data');
        } else {
          throw new Error('TMDB upcoming API returned no data');
        }
      } catch (upcomingError) {
        console.warn('⚠️ TMDB upcoming API failed, using notification data:', upcomingError.message);
        upcomingData = getUpcomingReleases().slice(0, 4);
      }

      setUpcomingMovies(upcomingData);

      // Update stats - use real data when available
      const watchlistCount = isAuthenticated ? 23 : 0;
      const totalMoviesCount = trendingData.movies.length > 0 ?
        (usingFallbackData ? 1247 : Math.floor(Math.random() * 5000) + 2000) : 1247;
      ...
      // Load fallback data
      const fallbackData = await movieService.getTrendingMovies();
      setTrendingMovies(fallbackData.movies);
      setUpcomingMovies(getUpcomingReleases().slice(0, 4));
      setUsingFallbackData(true);
    } finally {
      setLoading(false);
    }
  ...
  const handleSearch = useCallback(async (query) => {
    ...
    try {
      ...
      let searchResults;
      try {
        const tmdbSearchResult = await externalApiService.searchTMDBMovies(query, 1);
        if (tmdbSearchResult.success && tmdbSearchResult.movies.length > 0) {
          searchResults = { movies: tmdbSearchResult.movies };
          console.log('✅ Using TMDB search results');
        } else {
          throw new Error('TMDB search returned no results');
        }
      } catch (tmdbError) {
        console.warn('⚠️ TMDB search failed, falling back to local search:', tmdbError.message);
        searchResults = await movieService.searchMovies(query);
      }
    ...
    >
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-red-800 font-medium">API Connection Issue</p>
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              {!usingFallbackData && !selectedMood && !searchQuery && (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {isAuthenticated ? (
            <>
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/genre/trending')}>
              ...
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/settings')}>
              ...
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/community')}>
              ...
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/genre/top-rated')}>
            ...
            <>
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/signup')}>
                <FaUsers className="text-3xl text-primary-600 group-hover:text-primary-700 mx-auto mb-3 transition-colors" />
              ...
              <button className="card-hover p-6 text-center group" onClick={() => navigate('/login')}>
                <FaFilm className="text-3xl text-primary-600 group-hover:text-primary-700 mx-auto mb-3 transition-colors" />
        ...
        {isAuthenticated && user?.role === 'admin' && (
          <div className="mt-8">
            <h3 className="text-xl font-bold text-gray-900 mb-4">Admin Quick Access</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <button
                className="card-hover p-6 text-center group bg-gradient-to-br from-red-50 to-red-100 border-red-200"
                onClick={() => navigate('/admin')}
              >
                <FaUserShield className="text-3xl text-red-600 group-hover:text-red-700 mx-auto mb-3 transition-colors" />
                <span className="font-medium text-red-700">Admin Panel</span>
              </button>
              <button
                className="card-hover p-6 text-center group bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200"
                onClick={() => navigate('/admin/users')}
              >
                <FaDatabase className="text-3xl text-purple-600 group-hover:text-purple-700 mx-auto mb-3 transition-colors" />
                <span className="font-medium text-purple-700">User Database</span>
              </button>
              <button
                className="card-hover p-6 text-center group bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200"
                onClick={() => navigate('/moderator')}
              >
                <FaShieldAlt className="text-3xl text-yellow-600 group-hover:text-yellow-700 mx-auto mb-3 transition-colors" />
                <span className="font-medium text-yellow-700">Moderator Panel</span>
              </button>
              <button
                className="card-hover p-6 text-center group bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200"
                onClick={() => navigate('/admin/database')}
              >
                <FaCog className="text-3xl text-blue-600 group-hover:text-blue-700 mx-auto mb-3 transition-colors" />
                <span className="font-medium text-blue-700">Database Admin</span>
              </button>
            </div>
          </div>
        )}
