...
class MovieService {
  constructor() {
    ...
    this.useSampleData = process.env.REACT_APP_USE_SAMPLE_DATA === 'true'; // Default to false for real API usage
  ...
  async getTrendingMovies() {
    ...
    // Try TMDB API first
    try {
      const tmdbResult = await externalApiService.getTMDBTrendingMovies('week', 1);
      if (tmdbResult.success && tmdbResult.movies.length > 0) {
        return {
      ...
      console.warn('TMDB trending movies failed:', tmdbError);
    }
    ...
    try {
      ...
      console.warn('Local trending movies failed:', localError);
    }

    // Final fallback to sample data
    console.warn('All trending movie sources failed, using sample data');
    const movies = getSampleTrendingMovies();
    return { movies, period: "week", source: "sample" };
  }
